...
const LinkNavigation = ({ titulo, src, alt, index }) => {
  ...
  return (
    <>
      <Link href={''}> {/* Link pode ser passado como props, ou utilizar '/${item}' */}
        <li className=' px-4 border-b-2 h-[80px] flex items-center gap-2' onClick={() => handleDropBoxToggle(index)}>
          <Image
            src={src}
            height={25}
            width={25}
      ...
      {index === 2 || index === 3 ? <DropBox lista={index === 2 ? generos : decadas} open={openDropBox === index} /> : ''}
    </>
  )
